define([
  'require',
  'exports',
  '../../patchwork/core/EventDispatcher',
  './ConnectionsSelectionGrid',
  '../event/ConnectionsCanvasEvent',
], function(
  require,
  exports,
  EventDispatcher_1,
  ConnectionsSelectionGrid_1,
  ConnectionsCanvasEvent_1,
) {
  'use strict';
  class ConnectionsCanvas extends EventDispatcher_1.default {
    constructor(viewOffset) {
      super();
      this.drawDebugSelectionGrid = false; // draws the connection selection grid
      this.connectionMouseOverIndex = -1;
      this.viewOffset = viewOffset;
      this.connectionSelectionGrid = new ConnectionsSelectionGrid_1.default(
        10,
        this.drawDebugSelectionGrid,
      );
      ...
      this.$canvas.on(
        ...
        function(event) {
          ...
          var connectionMouseOverIndex = this.connectionSelectionGrid.getConnectionIndexAt(
            event.clientX,
            event.clientY,
          );
      ...
      this.$canvas.on(
        ...
        function(event) {
          if (this.connectionMouseOverIndex >= 0)
            this.dispatchEvent(
              ConnectionsCanvasEvent_1.default.CONNECTION_CLICKED,
              { connectionIndex: this.connectionMouseOverIndex },
            );
        }.bind(this),
      );
    }
    ...
    clear() {
      ...
      this.connectionSelectionGrid.create(
        this.canvas.width,
        this.canvas.height,
      );
      //var margin = 10;
    ...
    draw(patch) {
      ...
      for (var i = 0; i < connections.length; i++) {
        lineObjects = lineObjects.concat(
          this.createLineSegments(connections[i], i),
        );
      }
      // then draw all lines
      ...
      if (this.drawDebugSelectionGrid) {
        ...
        for (
          var i = 0;
          i < this.connectionSelectionGrid.locations.length;
          i++
        ) {
    ...
    createLineSegments(connection, index) {
      var segments = [];
    ...
    drawWithCreation(patch, mouseX, mouseY) {
      ...
      if (this.connectionCreationData) {
        ...
        if (this.connectionCreationData.source) {
          ...
          var $transputElement = this.findTransputByModuleIdAndIndex(
        ...
        lineObject.fromX =
          transputElementOffset.left + 0.5 * $transputElement.outerWidth();
        lineObject.fromY =
          transputElementOffset.top + 0.5 * $transputElement.outerHeight();
    ...
    drawLine(lineObject) {
      ...
      if (lineObject.end === true) {
        ...
        this.context.lineTo(
          lineObject.toX - arrowLength,
          lineObject.toY - 0.5 * arrowWidth,
        );
        this.context.lineTo(
          lineObject.toX - arrowLength + arrowBack,
          lineObject.toY,
        );
        this.context.lineTo(
          lineObject.toX - arrowLength,
      ...
      this.connectionSelectionGrid.drawLine(
        lineObject.connectionIndex,
        lineObject.fromX,
        lineObject.fromY,
        lineObject.toX,
        lineObject.toY,
    ...
    convertToCanvasSafeNumberfunction(value) {
      return Math.round(value) + 0.5;
    }
    findTransputByModuleIdAndIndex(moduleId, index, type) {
